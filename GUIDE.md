...
### 一键开始开发
```bash
# Spec-Kit 规范驱动开发 - 从需求到实施的完整流程
/spec-kit:specify "设计一个支持多租户的SaaS系统"
```
...
```bash
/spec-kit:specify "功能描述"
# ↓ 6步骤流程
# 规格创建 → 需求澄清 → 实施计划 → 任务生成 → 质量分析 → 执行实施
```
...
3. **充分描述需求**
   ```bash
   # ✅ 详细描述
   /spec-kit:specify "实现用户认证系统，支持JWT、OAuth2、记住登录状态"

   # ❌ 描述过于简单
   /spec-kit:specify "登录功能"
   ```
...
```bash
# 复杂项目的完整流程
/spec-kit:specify "设计用户管理系统"     # 先做规范设计
# ↓ 获得需求和设计文档后
/spec-kit:implement  # 按任务清单自动实现
```
