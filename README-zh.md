...
```bash
/ask              # 高级系统架构师咨询
                  # - 多专家咨询（系统设计师、技术策略师、可扩展性顾问、风险分析师）
                  # - "UltraThink"反思方法论进行全面分析
                  # - 战略技术指导，包含优缺点分析
                  # - 纯架构指导，无代码变更
```
...
/debug            # UltraThink调试协调器
                  # - 4阶段系统化调试（问题分析→多智能体→UltraThink→验证）
                  # - 多智能体协调（架构师、研究员、编码员、测试员专家）
                  # - 假设生成与精炼（5-7个→1-2个最可能解决方案）
                  # - 实施修复前的用户确认门控
                  # - 基于证据的诊断策略
```
...
/code             # 有约束的功能实现
/review           # 多维度代码审查
                  # - 4个专业审查员（质量审计员、安全分析师、性能审查员、架构评估员）
                  # - 带工作量估算的优先级可操作反馈
                  # - 专注实用质量而非架构完美
...
#### **🎯 完整自动化工作流**
```bash
/requirements-pilot # 需求驱动开发流水线
                    # - 交互式需求确认，100分质量评估
                    # - 90+质量分数时的用户批准门控
                    # - 4阶段实现：需求→规格→代码→测试
                    # - 智能测试决策（--test/--no-test标志）
...
# UltraThink调试工作流
/debug "生产环境API超时问题"
# → 生成5-7个假设，协调4个专家，精炼为前2个解决方案
# → 实施修复前请求用户确认

# 多专家架构咨询
/ask "高性能API网关的设计考虑"
# → 系统设计师分析边界，技术策略师评估选择
# → 可扩展性顾问评估性能，风险分析师识别权衡

# 带质量门控的需求驱动开发
/requirements-pilot "JWT认证支持刷新令牌" --test
# → 交互式澄清直到90+质量分数
# → 实施前用户批准门控
# → 自动化规格→代码→审查→测试流水线
```
...
- **优化工具**: `/optimize` - 性能优化协调
- **需求工具**: `/requirements-pilot` - 完整流水线，质量门控和批准工作流

#### **Agents 目录** (`/agents/`)
- **requirements-generate**: 代码生成优化的技术规格，直接实现映射
- **requirements-code**: 最小架构开销的直接实现智能体
- **requirements-review**: 专注功能性而非架构完美的实用审查（3级评分）
