...
|---------|------|----------|----------|----------|----------|
| **核心工作流** | `/requirements-pilot` | 快速开发工作流 | 端到端快速实现 | 完整解决方案 | 集成多个环节 |
| | `/kiro` | 规范驱动开发工具 | 结构化功能开发 | 自适应专家服务 | 智能路由和状态管理 |
| | `/bmad-pilot` | AI团队协作工作流 | 复杂功能团队开发 | 企业级完整方案 | 多智能体深度协作 |
| **规划与分析** | `/ask` | 技术咨询 | 架构设计咨询 | 专业建议 | 辅助决策制定 |
| | `/story-breakdown` | 功能分解 | 复杂功能拆分 | 用户故事列表 | 为其他命令提供输入 |
| | `/plan` | 计划制定模式 | 复杂实施计划 | 3阶段结构化计划 | 实施前详细规划 |
| | `/ultrathink` | MECE结构化深度分析 | 重大技术决策/战略规划 | 深度分析报告 | 为重大决策提供依据 |
| | `/multi-role` | 并行分析 | 综合专业评估 | 多维分析报告 | 整合专业视角 |
| **开发实现** | `/code` | 直接实现 | 明确需求的功能 | 代码文件 | 独立执行 |
| | `/refactor` | 重构实施 | 代码质量改进 | 重构后代码 | 配合/review使用 |
| | `/bugfix` | Bug修复 | 具体错误修复 | 修复方案 | 直接解决问题 |
|         |         |       | 多层次智能检测 | v2.0简化验证 | 框架特定错误 |
| | `/debug` | 系统化调试 | 复杂问题分析 | 调试分析 | 深度问题诊断 |
| | `/optimize` | 性能优化 | 性能瓶颈解决 | 优化方案 | 专业性能改进 |
| | `/translate` | 多语言翻译 | 国际化支持 | 翻译内容 | 辅助工具 |
| **质量保证** | `/review` | 代码审查 | 发现问题建议 | 审查报告 | 为/refactor提供输入 |
| | `/test` | 测试验证 | 功能质量验证 | 测试结果 | 验证其他命令结果 |
| | `/release-check` | 生产就绪验证 | 发布前检查 | go/no-go决策 | 质量门控 |
| **信息获取** | `/search-gemini` | 网络搜索 | 最新信息获取 | 搜索结果分析 | 补充知识base |
| | `/ctx7` | 技术文档搜索 | MCP Context7搜索 | 技术文档内容 | 技术调研和问题解决 |
| | `/role-debate` | 对立辩论 | 技术选型争议 | 辩论结果 | 帮助技术决策 |
| **辅助工具** | `/screenshot` | 屏幕截图分析 | UI/UX评估 | 界面分析报告 | 可视化分析 |
| | `/git-cm` | Git操作流程 | 智能提交推送 | Git操作结果 | 版本控制自动化 |
| | `/git-msg` | 提交信息生成 | 规范提交信息 | 提交信息 | Git工作流辅助 |
| | `/session-review` | 会话分析 | 学习优化 | 分析报告 | 持续改进 |
